...
function Script:main() {
  ...
  init)
    ...
    if [[ ! -f "$REDEPLOY" ]] ; then
      ...
      create_redeploy_script > "$REDEPLOY"
      chmod +x "$REDEPLOY"
    fi
...
function create_redeploy_script() {
  local folder
  folder="$(pwd)"
  echo "#!/usr/bin/env bash"
  echo "## automatically generated by $script_prefix $script_version"
  echo "cd '$folder' || exit"
  echo "[[ ! -d '$LOG_DIR' ]] && mkdir -p '$LOG_DIR'"
  echo "TODAY=\$(date '+%Y-%m-%d')"
  echo "LOGFILE=\"$LOG_DIR/redeploy.\$TODAY.log\""
  ...
  case $ENVIRONMENT in
    php)
      ...
      echo "if [[ -f composer.lock ]] ; then"
      echo "  {"
      echo "  echo \"##### composer install @ \$(date)\""
      echo "  \$PHP_BIN \$COMPOSER_BIN install --no-interaction --prefer-dist --optimize-autoloader"
      echo "  } &>> \"\$LOGFILE\""
      echo "fi"
      echo " "
      echo "if [[ -f artisan ]] ; then"
      echo "  #### PHP Laravel commands"
      echo "  {"
      echo "  echo \"##### artisan migrate @ \$(date)\""
      echo "  \$PHP_BIN artisan migrate --force"
      echo "  echo \"##### artisan cache:clear @ \$(date)\""
      echo "  \$PHP_BIN artisan cache:clear"
      echo "  echo \"##### artisan config:cache @ \$(date)\""
      echo "  \$PHP_BIN artisan config:cache"
      echo "  echo \"##### artisan route:cache @ \$(date)\""
      echo "  \$PHP_BIN artisan route:cache"
      echo "  echo \"##### artisan view:cache @ \$(date)\""
      echo "  \$PHP_BIN artisan view:cache"
      echo "  echo \"##### artisan queue:restart @ \$(date)\""
      echo "  \$PHP_BIN artisan queue:restart"
      echo "  } &>> \"\$LOGFILE\""
      echo "fi"
      ;;
    *)
